...
4. **Analyze per category** — see `references/categories.md` for the full list and what to look for in the scan data. For a large project, the NDJSON shape lets you delegate per session (or a batch of sessions) to a subagent, passing just the relevant lines rather than the whole file.
